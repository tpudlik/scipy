...
import numpy as np
from numpy.testing import dec, run_module_suite, assert_, assert_almost_equal
...
def test_hyp1f1_hard_points_real():
    # Unless noted otherwise, the points are from Table A.1 in
    ...
    dataset = [p + (float(mpmath.hyp1f1(*p)),) for p in pts]
...
def test_hyp1f1_hard_points_complex():
    # Unless noted otherwise, the points are from Table A.1 in
    # http://arxiv.org/abs/1407.7786
    pts = [
        (0.1, 0.2, -0.5 + 1j),
        (1 + 1j, 1 + 1j, 1 - 1j),
        (10**(-8), 10**(-12), -10**(-10) + 10**(-12)*1j),
        (1, 1, 10 + 10**(-9)*1j),
        (50, 10, 200j),
        (-5, (-5 + 10**(-9))*(1+1j), -1),
        (5, 0.1, -2 + 300j),
    ...
    dataset = [p + (complex(mpmath.hyp1f1(*p)),) for p in pts]
    dataset = np.array(dataset, dtype=np.complex_)
